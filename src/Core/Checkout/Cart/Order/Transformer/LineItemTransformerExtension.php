...
namespace LandimIT\Subscription\Core\Checkout\Cart\Order\Transformer;
...
class LineItemTransformerExtension
{
    ...
    public static function transform(LineItem $lineItem, ?string $parentId = null, int $position = 1): array
    {
        ...
        if ($lineItem->getExtension('subscription')) {
            $data['subscription'] = $lineItem->getExtension('subscription')->getMyCustomData();
        }
